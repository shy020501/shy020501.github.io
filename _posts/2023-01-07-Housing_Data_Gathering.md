...
* 메서드 설명:
  * np.random.permutation(): 무작위로 섞인 배열 생성하는 메서드
  * iloc(): 행/열을 숫자로 location을 나타내서 selecting/indexing하는 메서드
...
* 메서드/연산자들 설명:
  ...
  * apply(): 행 또는 열 또는 전체 셀에 원하는 연산을 지원하는 메서드
  * loc(): label이나 조건표현으로 선택하는 메서드
  * '~' 연산자: 비트를 반전시키는 비트 보수 연산자 (i.e. ~ 0111 1010 => 1000 0101)
  * reset_index(): 인덱스를 열로 변환하는 메서드
...
* 이를 달성하기 위해, pd.cut()을 사용하여 5개로 나누어진 소득 카테고리 특성을 만들 수 있습니다(카테고리 1은 median income에서 0~1.5, 카테고리 2는 median income에서 1.5~3, ...).
...
* 메서드 설명:
  * split(): 훈련 세트와 테스트 세트로 분할하기 위한 인덱스를 생성해 반환하는 메서드
  * value_counts(): 지정된 열의 값에 대한 발생 횟수를 반환하는 메서드
